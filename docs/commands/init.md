...
`--project-version` sets the plugin's own `project.version`. The Minecraft
version lives at `compatibility.versions[0]` and is set by `--mc-version`.
The global `-V, --version` prints pluggy's version and exits, which is why
the project's version has the longer name.
