...
import { parseMcVersion, parseSemver, platformListOption } from "./parsers.ts";
...
export async function generateProject(
  ...
  try {
    await appendGitignoreRules(join(distDir, ".gitignore"));
  } catch (e) {
...
/**
 * Add pluggy's ignore rules, keeping whatever is already there.
 *
 * This used to overwrite unconditionally. Running `init` in a directory that
 * already had a `.gitignore` — a repo you were adding a plugin to, or a
 * mistyped path — silently replaced it, un-ignoring everything the project
 * depended on being ignored.
 */
async function appendGitignoreRules(path: string): Promise<void> {
  ...
  if (existing === undefined) {
    await writeFileLF(path, `${GITIGNORE_RULES.join("\n")}\n`);
    return;
  }

  const present = new Set(existing.split(/\r?\n/).map((line) => line.trim()));
  const missing = GITIGNORE_RULES.filter(
    (rule) => rule !== "" && !rule.startsWith("#") && !present.has(rule),
  );
  if (missing.length === 0) return;

  const separator = existing.endsWith("\n") ? "" : "\n";
  await writeFileLF(path, `${existing}${separator}\n# pluggy\n${missing.join("\n")}\n`);
}
 ...
 */
/**
 * Reject a `--mc-version` the selected platforms don't publish. Network
 * failures are not fatal: `--mc-version` exists partly so a project can be
 * scaffolded offline, so an unreachable registry falls through to accepting
 * the value.
 */
async function assertPublished(version: string, selectedPlatforms: string[]): Promise<void> {
  let published: Array<{ platform: string; versions: string[] }>;
  try {
  ...
  const missing = published.filter((entry) => !entry.versions.includes(version));
  if (missing.length === 0) return;

  const entry = missing[0];
  if (entry === undefined) return;
  throw new UserError(`${entry.platform} does not publish Minecraft ${version}.`, {
    code: "E_INIT_UNKNOWN_MC_VERSION",
    hint: `Run \`pluggy platforms ${entry.platform}\` to see every version it publishes.`,
    context: { version, platforms: missing.map((m) => m.platform) },
  });
}

/**
 * Reject an unknown `--template` id up front, naming the ones that exist.
 *
 * The load path reported `Template "x" not found in fetched zip` — after a
 * network round trip, leaking an implementation detail, and listing none of
 * the templates that do exist.
 */
async function assertTemplateExists(id: string): Promise<void> {
  let available: Awaited<ReturnType<typeof listTemplates>>;
  try {
  ...
  throw new UserError(`No template named "${id}".`, {
    code: "E_INIT_UNKNOWN_TEMPLATE",
    hint: `Available: ${available.map((t) => t.id).join(", ")}. Run \`pluggy templates\` for descriptions.`,
    context: { template: id, available: available.map((t) => t.id) },
  });
}
...
export function initCommand(): Command {
  return new Command("init")
    ...
    .addOption(
      new Option("--platform <ids>", "Target platform (repeatable; comma-separated).")
        .argParser(platformListOption)
    ...
    .action(async function action(this: Command, path: string | undefined, options) {
      ...
      if (options.template !== undefined) {
        await assertTemplateExists(options.template);
        try {
      ...
      if (options.mcVersion) {
        // Shape-checking alone let `--mc-version 9.9.9` scaffold a project that
        // only failed three commands later, as a raw Maven 404. Check it against
        // what the platforms actually publish — but stay usable offline, which
        // is the reason this flag skips the lookup in the first place.
        await assertPublished(options.mcVersion, selectedPlatforms);
